...
import { web3Accounts, web3AccountsSubscribe, web3Enable } from '@talismn/connect'
import { get } from 'lodash'
import { createContext, useContext, useEffect, useMemo, useReducer, useState } from 'react'

import { useAwaitObjectValue, useStatus } from './util/hooks'
...
const Provider = ({ children }) => {
  ...
  const { status, message, setStatus, options } = useStatus({
    status: 'PROCESSING',
    message:
      'This website requires that the <a href="https://polkadot.js.org/extension/" target="_blank">polkadot.js extension</a> is installed.',
    customOptions: {
  ...
  const initApplication = async () => {
    console.log('init')
    try {
      console.log('process.env.REACT_APP_APPLICATION_NAME', process.env.REACT_APP_APPLICATION_NAME)
      const injected = await web3Enable(process.env.REACT_APP_APPLICATION_NAME)
      console.log('accoutns', await web3Accounts())
  ...
  const hydrateAccounts = async () => {
    ...
    const accountSub = await web3AccountsSubscribe(accounts => {
      ...
      if (accounts.length <= 0) {
        setStatus(
          'NOACCOUNT',
          'Please create an account/address in the polkadot.js extension to be able to interact with this application'
        )
      } else {
        ...
        accountDispatcher({
          ...
          callback: () =>
            setStatus(
              'AUTHORIZED',
              'The polkadot.js extension is installed and authorized, and accounts have been found'
            ),
        })
  ...
  useAwaitObjectValue(
    ...
    'injectedWeb3.talisman', // for this keys' value (dot delimited - ala lodash get)
  ...
  useEffect(
    () => !!api.isReady && status === options.AUTHORIZED && initBalanceSubscriptions(),
    [api.isReady, Object.values(accountsByAddress).length]
  ) // eslint-disable-line
